...
def RunIfFailed(job):
  return "{} == 'failure'".format(
      GitHubExpr("needs.{jobid}.result".format(jobid=job.id())))
...
def ccache(build, env):
  ...
  build = [
      ...
      GHCache(),
      CcacheEnv(
          check=GitHubExpr('env.cache_cmd'),
...
def ubuntu():
  ...
  build_epilog = [
      ...
      ImportedSnippet(
          ...
          working_directory='build',
          condition="{} == 'false'".format(GitHubExpr('env.unittests'))),
  ]
  ...
  for env_name, env in envs.items():
    platform = {
        "id": "ubuntu_{}".format(env_name),
        "name": "Ubuntu {}".format(env_name),
        "os": "ubuntu-18.04"
    }
